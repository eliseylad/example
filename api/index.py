from flask import Flask, request, send_file, render_file
...
def generate():
    if 'object' not in request.files or 'texture' not in request.files:
        return {"error": "Выберите оба файла"}, 400
    ...
    if result_buffer is None:
        return {"error": "Ошибка обработки"}, 400
    
    return send_file(result_buffer, mimetype='image/png', as_attachment=True, download_name='mockup.png')
